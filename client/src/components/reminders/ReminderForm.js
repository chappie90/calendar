...
class ReminderForm extends React.Component {
	...
	renderDateInput = ({ input, label, meta }) => {
		const className = `field ${meta.error && meta.touched ? 'error': ''}`;
			return (
				<div className={className}>
					<label>{label}</label>
					<input {...input} value={this.state.editDate} type="date" min={dateFns.format(new Date(), 'YYYY-MM-DD')} />
					{this.renderError(meta)}
				</div>
			);
	}
	...
	renderSelectTime = ({ input, label, meta }) => {
		...
		for (let i = 0; i < 24; i++) {
			if (i < 10) {
				time.push(
					<option key={i} value={`0${i}:00`}>0{i}:00</option>
				);
			} else {
				time.push(
	...
	render() {
		return (
			<form onSubmit={this.props.handleSubmit(this.onSubmit)} className="ui form">
				<Field onChange={this.onInputChange} name="date" component={this.renderDateInput} label="Select date" /><br />
				<Field name="time" component={this.renderSelectTime} label="Select time" /><br />
				<Field name="reminder" component={this.renderTextInput} label="Enter reminder" /><br />
